...
| Sole-owner map writes | the xlang wordfreq benchmark: `m = map_set(m, k, v)` clones the whole map per insert on both tiers even when the move fusion makes the argument sole-owner, so hot string-keyed counting is quadratic — the one benchmark olang cannot finish; `collections.table` is no rescue once the calling loop promotes (every put crosses the tier boundary converting the handle, the C7 T2 lane) | planned — a `MapSetAssign` fusion with the list writes' sole-owner discipline (in place when unaliased, copy when shared), VM and interpreter both; the JIT's map_set helper follows the ListSetAssign ownership-family pattern |
