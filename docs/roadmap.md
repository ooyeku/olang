...
| DP4 — flagship and benchmark | A realistic end-to-end ETL example wired to `plot`, and a reproducible benchmark against pandas and Polars with methodology and hardware documented in [The data stack](ods.md). | **shipped** — `benchmarks/`: the same eight-stage ETL pass in ods, pandas, and Polars, checksum-verified per stage before any timing is reported; ods 145 ms end-to-end vs pandas 305 ms vs Polars 35 ms at 1M rows (first published at 223 ms; drop_null and frame filter closed most of their gap after); methodology, hardware, and the remaining load-path work in ods.md |
