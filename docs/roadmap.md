...
| # | Lane | What it does | Status |
|---|---|---|---|
| DP1 | **Parallel by default** — partition `group_by`, `join`, and elementwise column operations across real threads, so a large transform uses every core with no ceremony. This is the concrete advantage over pandas (single-threaded under the GIL) and the reason the niche is winnable | **join + group_by landed (0.60)** — the hash-join probe runs across all cores on a 50k+-row left frame, chunks concatenated in row order so the result is bit-identical to sequential (~2.6× on a 4M-row join). **group_by aggregation (DP1b)** scatters `sum`/`count`/`min`/`max` into per-group partials over fixed row ranges, merged in chunk order: ints bit-identical, float `sum`/`mean` reorder but reproducibly (fixed chunk layout); ~1.4× on 8M rows / 1k groups. Elementwise Series ops were already parallel. Both pinned by tests in both feature modes. **Next: DP1c** — parallelize the group-id hashing pass, which now dominates group_by |
