...
| # | Lane | What it does | Status |
|---|---|---|---|
| DP1 | **Parallel by default** — partition `group_by`, `join`, and elementwise column operations across real threads, so a large transform uses every core with no ceremony. This is the concrete advantage over pandas (single-threaded under the GIL) and the reason the niche is winnable | **join landed (0.60)** — the hash-join probe runs across all cores on a 50k+-row left frame, chunks concatenated in row order so the result is bit-identical to sequential (pinned by a test in both feature modes); measured ~2.6× on a 4M-row join. Elementwise Series ops were already parallel. **group_by aggregation is next (DP1b)** — its float reduction needs a deterministic order, handled deliberately |
