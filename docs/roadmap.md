...
| DP4 — flagship and benchmark | A realistic end-to-end ETL example wired to `plot`, and a reproducible benchmark against pandas and Polars with methodology and hardware documented in [The data stack](ods.md). | **shipped** — `benchmarks/`: the same eight-stage ETL pass in ods, pandas, and Polars, checksum-verified per stage before any timing is reported; ods 126 ms end-to-end vs pandas 313 ms vs Polars 34 ms at 1M rows (first published at 223 ms; drop_null, frame filter, and a borrowed CSV read path closed most of the gap after); methodology, hardware, and the remaining load-path work in ods.md |
...
| T3 — on-stack replacement | A hot loop compiles at its back-edge, so a once-called function's big loop doesn't live on the VM. | **shipped** — at 8192 back-edges a frame's single loop region is synthesized into a standalone function (live-in registers as parameters, live-outs returned, tuples for several) and compiled by the ordinary JIT; the dispatch loop enters it mid-frame and resumes at the loop's exit. Any native failure resumes the VM at the head with untouched registers, so errors re-raise with original spans. 7× on the arithmetic-loop repro; four regression tests pin entry, tuple write-back, moved-list conversion, and error identity |
| T4 — JIT call inlining | Small callees inline into their JIT callers; the loop-calling-a-function shape stops paying a boundary per iteration. | **shipped** — inlining is transitive to two levels: a callee whose own calls inline away presents its expanded call-free form and inlines like a leaf (recursive callees bottom out and stay calls); whitelisted named/builtin calls pass through the splice. Depth-3 helper chain: 153ms → 82ms on the 30M-iteration repro; two differential tests pin agreement |
...
| Lane | Work | Status |
|---|---|---|
| E1 — polymorphic specialization | A function is no longer one kind-vector specialization forever: a call with new argument kinds compiles an additional variant (capped per function), so mixed-type call sites stay native instead of falling off after the first shape. | **shipped** — a mismatched shape compiles its own group beside the primary (three variants per function, refused shapes never retry; every group compile carries a unique symbol tag). Group planning adopts existing variants for direct native-to-native calls and plans fresh ones when a callee is compiled on other kinds — which also unfroze groups that used to refuse outright on a callee kind mismatch. The mixed Int/Float repro: 100k → 200k native calls, 300k → 0 VM instructions; pinned by a tier floor |
...
| E3 — JIT strings | String operations in native code beyond compare/concat — the template-building shape (report and CSV builders) stops being an automatic refusal. | **shipped** — `MakeTemplate` joins the whitelist: literal parts are baked borrowed pointers into the owned bytecode, scalar parts stringify through a host helper with the interpreter's exact rules (`to_string`, `format_float`), and parts fold through the existing concat discipline, watermark rules included. The report shape (a hot loop ending in a template): 13.5M VM instructions → 0, 0 → 300 native calls, 3.3× wall-clock; pinned by a tier floor |
| E5 — build-embedded warm profiles | `olang build` embeds the source's warm profile into the artifact, so a built program starts with its tier knowledge in hand — no sidecar directory, no cold first run. The sidecar path (shipped) stays for `olang run`. | **shipped** — run once, then build: the bundle meta carries the sidecar profile (provenance, not integrity — outside the digest like the platform fields), the embedded runner installs it at startup, and a sidecar on the running machine still wins as fresher evidence. Integration test pins carry + replay + identical output on a fresh warm directory |
| E7 — data-stack fusion | Elementwise Series chains (`f["a"] * f["b"] + 1.0`) evaluate as one fused loop instead of materializing per verb — closing the remaining engine-side gap to the columnar competition. | **shipped** — invisible chain fusion at the operator seam: `+`/`-`/`*` on null-free Float operands builds a deferred expression, forced in one chunked (and parallel) pass at first observation; bit-identical by construction (same per-element FP ops in the same order), with division, nulls, and mixed dtypes on the eager path unchanged. 2.7× on a four-op chain over 5M elements, identical checksum; the ods.md eager-evaluation record carries the amendment |
