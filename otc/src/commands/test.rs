use anyhow::anyhow;
use olang::test_framework::{TestError, TestRunner};
use std::path::Path;
...
fn run_tests_once(
    ...
    let total_tests: usize = test_runner
        .test_suites
        .iter()
    ...
    if verbose {
        println!(
            "   Found {} test suites with {} tests total",
...
fn filter_tests(test_runner: &mut TestRunner, pattern: &str, verbose: bool) {
    let original_count: usize = test_runner
        .test_suites
        .iter()
        .map(|suite| suite.tests.len())
        .sum();
    ...
    test_runner
        .test_suites
        .retain(|suite| !suite.tests.is_empty());

    let filtered_count: usize = test_runner
        .test_suites
        .iter()
        .map(|suite| suite.tests.len())
        .sum();
