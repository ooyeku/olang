...
pub fn config_list(verbose: bool) -> Result<()> {
    ...
    if verbose {
        println!("\nConfiguration file: {}", global_otc.config_file().display());
    }
...
pub fn cache_status(verbose: bool) -> Result<()> {
    ...
    println!("  Size: {} MB / {} MB ({:.1}%)", size_mb, max_size_mb, usage_percent);
...
pub fn system_info(verbose: bool) -> Result<()> {
    ...
    if verbose {
        ...
        println!("  Cache: {}", if info.cache_dir.exists() { "✓" } else { "✗" });
        println!("  Packages: {}", if info.packages_dir.exists() { "✓" } else { "✗" });
        
        println!("\nFile Status:");
        println!("  Config: {}", if info.config_file.exists() { "✓" } else { "✗" });
        println!("  Registry: {}", if info.registry_file.exists() { "✓" } else { "✗" });
    }
...
fn extract_package_name_from_url(url: &str) -> Result<String> {
    ...
    let name = if url.contains("github.com") || url.contains("gitlab.com") {
        // Extract from URLs like: https://github.com/user/package.git
        url.split('/')
            .last()
            .unwrap_or("unknown")
            .strip_suffix(".git")
        ...
        return Err(anyhow::anyhow!("Unable to extract package name from URL: {}", url));
    };
