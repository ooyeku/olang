...
fn test_boolean_patterns_not_exhaustive_missing_true() {
    ...
    assert!(
        ...
        "Should report missing true pattern"
    );
}
...
fn test_boolean_patterns_not_exhaustive_missing_false() {
    ...
    assert!(
        ...
        "Should report missing false pattern"
    );
}
...
fn test_result_patterns_not_exhaustive_missing_ok() {
    ...
    let missing = analyzer.get_missing_patterns(&patterns);
    assert!(
        missing.contains(&"Ok(_)".to_string()),
        "Should report missing Ok pattern"
    );
}
...
fn test_result_patterns_not_exhaustive_missing_err() {
    ...
    let missing = analyzer.get_missing_patterns(&patterns);
    assert!(
        missing.contains(&"Err(_)".to_string()),
        "Should report missing Err pattern"
    );
}
...
fn test_literal_patterns_not_exhaustive() {
    ...
    assert!(
        !result,
        "Literal patterns without wildcard should not be exhaustive"
    );

    let missing = analyzer.get_missing_patterns(&patterns);
    assert!(
        missing.contains(&"_ (wildcard pattern)".to_string()),
        "Should report missing wildcard pattern"
    );
}
...
fn test_or_pattern_with_boolean_exhaustive() {
    ...
    assert!(
        result,
        "Or pattern covering both boolean values should be exhaustive"
    );
}
...
fn test_or_pattern_with_catch_all_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Or {
        alternatives: vec![Pattern::Literal(Value::Integer(1)), Pattern::Wildcard],
    }];
...
fn test_guarded_patterns_not_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Guarded {
    ...
    assert!(
        !result,
        "Guarded patterns should not be considered exhaustive"
    );
}
...
fn test_tuple_patterns_not_exhaustive() {
    ...
    assert!(
        !result,
        "Tuple patterns without wildcard should not be exhaustive"
    );
...
fn test_list_patterns_not_exhaustive() {
    ...
    assert!(
        !result,
        "List patterns without wildcard should not be exhaustive"
    );
...
fn test_enum_patterns_not_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::EnumVariant {
    ...
    assert!(
        !result,
        "Enum patterns without all variants should not be exhaustive"
    );

    let missing = analyzer.get_missing_patterns(&patterns);
    assert!(
        missing.contains(&"_ (wildcard pattern or other enum variants)".to_string()),
...
fn test_mixed_patterns_not_exhaustive() {
    ...
    assert!(
        !result,
        "Mixed patterns without wildcard should not be exhaustive"
    );
...
fn test_struct_patterns_not_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Struct {
    ...
    assert!(
        !result,
        "Struct patterns without wildcard should not be exhaustive"
    );
...
fn test_range_patterns_not_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Range {
    ...
    assert!(
        !result,
        "Range patterns without wildcard should not be exhaustive"
    );
...
fn test_comprehensive_boolean_or_pattern_exhaustive() {
    ...
    let patterns = vec![
        Pattern::Literal(Value::Boolean(true)),
        Pattern::Or {
            alternatives: vec![Pattern::Literal(Value::Boolean(false))],
        },
    ];
    ...
    assert!(
        result,
        "Boolean patterns covering both true and false should be exhaustive"
    );
}
...
fn test_comprehensive_result_or_pattern_exhaustive() {
    ...
    let patterns = vec![
        Pattern::Ok(Box::new(Pattern::Wildcard)),
        Pattern::Or {
            alternatives: vec![Pattern::Err(Box::new(Pattern::Wildcard))],
        },
    ];
    ...
    assert!(
        result,
        "Result patterns covering both Ok and Err should be exhaustive"
    );
}
...
fn test_nested_or_patterns_exhaustive() {
    ...
    assert!(
        result,
        "Nested or patterns covering both boolean values should be exhaustive"
    );
}
...
fn test_rest_patterns_not_exhaustive() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Rest("rest".to_string())];
...
fn test_missing_patterns_with_catch_all() {
    let analyzer = Analyzer::new();
    let patterns = vec![Pattern::Literal(Value::Integer(1)), Pattern::Wildcard];

    let missing = analyzer.get_missing_patterns(&patterns);
    assert!(
        missing.is_empty(),
        "Should report no missing patterns when there's a catch-all"
    );
}
...
fn test_analyze_pattern_structure_boolean() {
    ...
    assert!(
        result,
        "Boolean pattern structure should be correctly analyzed"
    );
}
...
fn test_analyze_pattern_structure_result() {
    ...
    assert!(
        result,
        "Result pattern structure should be correctly analyzed"
    );
}
...
fn test_analyze_pattern_structure_mixed() {
    ...
    assert!(
        missing.contains(&"_ (wildcard pattern)".to_string()),
        "Should report missing wildcard pattern for mixed patterns"
    );
}
